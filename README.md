...
Để tránh lỗi phiên bản thư viện không tương thích với máy tính của bạn, chúng ta sẽ tạo một môi trường chạy riêng biệt và sạch sẽ cho ứng dụng:
...
3. Copy toàn bộ đoạn lệnh dưới đây, dán vào cửa sổ đen đó (nhấp chuột phải để dán trên CMD) rồi nhấn **Enter**:
   ```cmd
   python -m venv venv && venv\Scripts\activate && python -m pip install --upgrade pip && pip install --force-reinstall -r requirements.txt
